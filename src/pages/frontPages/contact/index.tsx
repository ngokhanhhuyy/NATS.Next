      ...
      <div className="container">
        <div className="row gx-5 gy-4 justify-content-center my-5">
          ...
          <div className="col col-md-10 col-sm-11 col-12">
            ...
            <p>
              Chúng tôi sẽ lắng nghe và phản hồi mọi thắc mắc của Quý khách trong vòng 24 giờ.
            </p>
            <p>
              Với đội ngũ lương y giàu kinh nghiệm và tận tâm trong lĩnh vực y học cổ truyền,{" "}
              {applicationNameElement} luôn đồng hành cùng Quý khách trên hành trình phục hồi
              sức khỏe xương khớp, giúp cơ thể dẻo dai và khỏe mạnh theo phương pháp tự nhiên,
              an toàn.
            </p>
          </div>
          ...
          <div className="col col-xl-6 col-sm-10 col-12 d-flex flex-column">
            {googleMapEncodedUrl && (
              <iframe
                width="100%"
                height="100%"
                className={`img-thumbnail rounded-4 shadow m-0 ${styles.googleMapFrame}`}
                style={{ border: "none" }}
                loading="lazy"
                allowFullScreen
  ...
  return (
    <div className={`d-flex flex-column ${columnClassName} ${styles.contactItem}`}>
      ...
      {props.model.type === ContactType.PhoneNumber && (
        <a href={`tel:${props.model.content}`}>{props.model.content}</a>
      )}
      ...
      {props.model.type === ContactType.Email && (
        <a href={`mailto:${props.model.content}`}>{props.model.content}</a>
      )}

      {props.model.type === ContactType.Address && (
        <a href={computeGoogleMapEncodedUrl()} target="_blank" rel="noopener noreferrer">
          {props.model.content}
        </a>
      )}
    </div>
