/**
 * Convert datetime string extracted from local-datetime HTMLInputElement to ISO 8601
 * format string (ICT timezone) to be used as data for request DTOs.
 *
 * @param dateTimeInputValue String value extracted from datetime-local HTMLInputElement.
...
/**
 * Convert date string extracted from date HTMLInputElement to ISO 8601 format string to be
 * used as data for request DTOs.
 *
...
/**
 * Convert time string extracted from time HTMLInputElement to ISO 8601 format string to be
 * used as data for request DTOs.
 *
...
/**
 * Convert datetime ISO string retrieved from the server to the format that is used as
 * local-datetime HTMLInputElement value. If the value is null, the result will be an empty
 * string.
 *
...
/**
 * Convert date ISO string retrieved from the server to the format that is used as date
 * HTMLInputElement value. If the value is null, the result will be an empty string.
 *
...
/**
 * Convert time ISO string retrieved from the server to the format that is used as time
 * HTMLInputElement value. If the value is null, the result will be an empty string.
 *
...
/**
 * Convert datetime ISO string in response DTOs retrieved from server to a string used for
 * dislaying.
 *
...
/**
 * Compare 2 dates given their ISO format strings. If the strings contain time parts, they
 * will be removed, only date parts are compared.
 *
 ...
 * @returns 1 if the target date is greater, 0 if they are equal, -1 if the compared date
 * is greater.
 */
...
/**
 * Compare 2 datetimes given their ISO format strings. If the strings contain only date
 * parts, they will be compared with time parts being 00:00:00.
 *
...
/**
 * Get the text describing the difference (delta) between the current datetime and the
 * datetime represented by the given string.
 *
...
/**
 * Get the months difference between the 2 given `Date` objects. The value will be rounded
 * down if it contains decimal part.
 *
...
/**
 * Get the years difference between the 2 given `Date` objects. The value will be rounded
 * down if it contains decimal part.
 *
 * @param pastDate A `Date` object.
 * @param futureDate A `Date` object which represents later date and time than the
...
/**
 * Get the `Date` object representing the date which is exactly 1 year different from the
 * given date. The result date will have the same month, same date (adjusted to the last
 * date of month if exceeds, same time).
 *
...
/**
 * Get a `Date` object representing the moment which is exactly 1 month earlier relative to
 * the given date.
 *
 * @param date A `Date` object to be based on.
 * @returns A `Date` object representing the moment which is exactly 1 month earlier.
 */
